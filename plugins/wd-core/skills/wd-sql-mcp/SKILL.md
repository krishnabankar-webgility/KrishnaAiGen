---
name: wd-sql-mcp
description: 'Use to set up or troubleshoot SQL Server access for a WD development session - the mssql-winauth (local dev, Windows auth) and mssql-sqlauth (restored customer databases, SQL auth) MCP connections.'
---

# wd-sql-mcp

The full instructions for this skill are maintained by the WD team inside the
`unify-enterprise` repo. **Read `.github/skills/sql-mcp-skill.md`** (relative to the repo root)
